...
# problem 1
"""
1) simulate in the AntColony class
2) the entrance and exit are both the same through which they will exit at the time specified in the ant colony
3) The number of places created depends on the number of ant_types which is a "list of all implemented Ant classes"
   To add more Places, we will have to add implement more Ants
4) Insect's armor attribute is the amount of health they have. The insects will be attacking each other which will reduce
    the armor attribute. When it hits 0 or less, the insect will be removed.
...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            # If a BodyguardAnt containing another ant is removed, then the ant it is containing should be placed where the BodyguardAnt used to be. Update the remove_insect method in the Place class accordingly.
            if isinstance(insect, QueenAnt) and insect.queen_number == 0 and insect.armor > 0:
                return
            elif insect.container == True and insect.ant is not None:
                self.ant = insect.ant
            else:
                self.ant = None
        insect.place = None
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if self.container == True and self.ant == None and other.container is not True:
            print('can contain')
            return True
        else:
            return False
        ...
        def check_place(place, hive):
            if place == hive:
                ...
                if bee is not None:
                    ...
                    return check_place(place.entrance, hive)

        k, place = 0, self.place
        while k < self.min_range:
            place = place.entrance
            if place == hive:
                return None
            else:
                k += 1
        return check_place(place, hive)
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        if self.queen_number < 1: # the class attribute increases but each instance gets its own unique queen_number

            # do her own damage to bees
            self.throw_at(self.nearest_bee(colony.hive))

            # double the damage of all other ants in the same tunnel including her own space
            place_exit = self.place.exit
            place_entrance = self.place.entrance
            def check_self(place):
                ant = place.ant
                if ant.container and ant not in QueenAnt.buffed_ants:
                    ant.damage *= 2

            def check_exit(place):
                if place is not None:
                    ant = place.ant
                    if ant is not None and ant not in QueenAnt.buffed_ants:
                        ant.damage *= 2
                ...
                if place is not None:
                    ...
                    if ant is not None and ant not in QueenAnt.buffed_ants:
                        ...
                        if ant.container and ant.ant is not None:
                            ant.ant.damage *= 2
                    place = place.entrance
                    check_entrance(place)
                else:
                    return

            check_self(self.place)
            check_exit(place_exit)
            check_entrance(place_entrance)

            # replace colony.queen with QueenPlace
